...
export interface ProcessJobInvoiceRequestAttributes {
    invoice_value: string,
    invoice_number: string,
    invoice_date: Date
}

const ProcessJobInvoiceRequest = (props: {
    ...
    const [processAttributes, setProcessAttributes] = useState<ProcessJobInvoiceRequestAttributes>({
    ...
    const updateDateParams = (event: ChangeEvent<HTMLInputElement>) => {
        updateStateDateParams(event, setProcessAttributes);
    }
    
    const processInvoiceRequest = () => {
        setHasSubmitted(true);
        if (!formComplete) return;
        putAPI(`job_invoice_requests/${props.jobInvoiceRequestID}/process`, {}, processAttributes, (response: any) => {
            const jobInvoiceRequestData: JobInvoiceRequestResponseData = response.data;
            props.setJobInvoiceRequestData(jobInvoiceRequestData);
            props.hideFunc();
        }, setIsUpdating)
    }
    ...
    return (
        <WindowOverlay 
            title={"Process Job Invoice Request"} 
            maxWidth={300} 
            show={props.show}
...
export default ProcessJobInvoiceRequest