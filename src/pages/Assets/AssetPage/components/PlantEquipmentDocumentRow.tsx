import ActionMenu, { ActionItem } from "../../../../components/form/ActionMenu/ActionMenu";
import { PlantEquipmentDocumentsResponseData } from "../../../../types/plantEquipmentDocument.types";
import formatDate from "../../../../utils/formatDate";
import PlantEquipmentDocumentLabel from "./PlantEquipmentDocumentLabel";
import PlantEquipmentDocumentLink from "./PlantEquipmentDocumentLink";

const PlantEquipmentDocumentRow = (props: {
    document: PlantEquipmentDocumentsResponseData,
    actions: Array<ActionItem>
}) => {
    return (
        <tr>
            <td className="text-left" style={{ width: '100%' }}>
                <PlantEquipmentDocumentLink
                    documentName={props.document.data.name}
                    fileName={props.document.data.file_name}
                    inactive={!props.document.data.is_active}
                />
            </td>
            <td><PlantEquipmentDocumentLabel type={props.document.data.type}/></td>
            <td>{formatDate(props.document.data.created_at)}</td>
            <td><ActionMenu actionItems={props.actions}/></td>
        </tr>
    )
}

export default PlantEquipmentDocumentRow