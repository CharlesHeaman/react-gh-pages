...
import getCustomerURL from "./utils/getCustomerURL"

const NewCustomerLink = (props: {
    code: string,
    name?: string,
    text?: string,
    ...
    return (
        <a 
            href={`${getCustomerURL(props.code)}${props.queryParameters ? `?${props.queryParameters}` : ''}`}
            className="icon-link"
        >
            {!props.inactive ?
                <span className="material-icons">{!props.listIcon ? 'groups' : 'wysiwyg'}</span> :
